      ...
      <div style={{flex:1,overflowY:"auto",padding:"0 16px 28px",display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:14}}>
            <div style={{position:"relative",width:90,height:90,flexShrink:0}}>
              <svg width={90} height={90} style={{transform:"rotate(-90deg)"}}>
                <circle cx={45} cy={45} r={36} fill="none" style={{stroke:"rgba(var(--surface-rgb),0.16)"}} strokeWidth={8}/>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:7}}>
            {signals.map((s,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:18,height:18,borderRadius:"50%",background:s.met?`${s.color}30`:"rgba(var(--surface-rgb),0.06)",border:`1px solid ${s.met?s.color:"rgba(var(--surface-rgb),0.12)"}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  <span style={{fontSize:9,color:s.met?s.color:"rgba(var(--surface-rgb),0.3)"}}>{s.met?"✓":"○"}</span>
                </div>
                <div style={{flex:1}}>
                  <div style={{display:"flex",justifyContent:"space-between"}}>
                    <span style={{fontFamily:"DM Sans,sans-serif",fontSize:11,color:s.met?C.pearl:"rgba(var(--ink-rgb),0.4)"}}>{s.label}</span>
                    <span style={{fontFamily:"DM Sans,sans-serif",fontSize:10,color:s.met?s.color:"rgba(var(--ink-rgb),0.3)",fontWeight:600}}>{s.impact}</span>
                  </div>
                  <div style={{height:3,borderRadius:2,background:"rgba(var(--surface-rgb),0.06)",marginTop:3}}>
        ...
        <Card style={{borderColor:`${C.gold}40`,background:`rgba(255,215,0,0.06)`}}>
          ...
          {[
            ...
            <div key={i} style={{padding:"8px 0",borderBottom:i<3?"1px solid rgba(var(--surface-rgb),0.05)":"none"}}>
        ...
        {AI_LAYERS.map((layer,i)=>(
          <Card key={layer.id} style={{cursor:"pointer",borderColor:expanded===i?`${layer.color}50`:`${layer.color}20`,background:expanded===i?`${layer.color}08`:"rgba(var(--velvet-rgb),0.55)"}}
            ...
            <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
              <div style={{position:"relative",width:48,height:48,flexShrink:0}}>
                <svg width={48} height={48} style={{transform:"rotate(-90deg)"}}>
                  <circle cx={24} cy={24} r={20} fill="none" style={{stroke:"rgba(var(--surface-rgb),0.16)"}} strokeWidth={4}/>
              ...
              <div style={{flex:1}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
                  ...
                  <div style={{display:"flex",gap:6,alignItems:"center"}}>
                    ...
                    <span style={{fontFamily:"DM Sans,sans-serif",fontSize:9,color:"rgba(var(--ink-rgb),0.4)"}}>{expanded===i?"↑":"↓"}</span>
                  </div>
                </div>
            ...
            {expanded===i&&<>
              <div style={{height:"1px",background:"rgba(var(--surface-rgb),0.07)",margin:"12px 0"}}/>
              ...
              <div style={{fontFamily:"DM Sans,sans-serif",fontSize:12,color:"rgba(var(--ink-rgb),0.75)",lineHeight:1.7,marginBottom:12}}>{layer.what}</div>
              ...
              <div style={{fontFamily:"DM Sans,sans-serif",fontSize:12,color:"rgba(var(--ink-rgb),0.65)",lineHeight:1.7,marginBottom:12}}>{layer.how}</div>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:12}}>
                {layer.inputs.map(inp=>(
                  <span key={inp} style={{padding:"3px 8px",borderRadius:10,background:`${layer.color}15`,border:`1px solid ${layer.color}30`,fontFamily:"DM Sans,sans-serif",fontSize:10,color:"rgba(var(--ink-rgb),0.7)"}}>{inp}</span>
                ))}
              </div>
              ...
              {layer.biomeSignals&&<>
                ...
                {layer.biomeSignals.map((sig,j)=>(
                  <div key={j} style={{padding:"8px",borderRadius:8,background:"rgba(var(--surface-rgb),0.04)",marginBottom:5}}>
                    ...
                    <div style={{height:4,borderRadius:2,background:"rgba(var(--surface-rgb),0.06)"}}>
                      <div style={{width:`${sig.confidence}%`,height:"100%",borderRadius:2,background:layer.color}}/>
                    </div>
                    <div style={{fontFamily:"DM Sans,sans-serif",fontSize:9,color:"rgba(var(--ink-rgb),0.35)",marginTop:2}}>{sig.confidence}% confidence</div>
                  </div>
                ))}
              </>}
              {layer.patterns&&!isLive&&<>
                ...
                {layer.patterns.map((p,j)=>(
                  <div key={j} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:j<layer.patterns.length-1?"1px solid rgba(var(--surface-rgb),0.05)":"none"}}>
                    <div>
                      <div style={{fontFamily:"DM Sans,sans-serif",fontSize:12,color:C.pearl}}>{p.symptom}</div>
                      <div style={{fontFamily:"DM Sans,sans-serif",fontSize:10,color:"rgba(var(--ink-rgb),0.45)"}}>{p.day} · {p.phase}</div>
                    </div>
                    <div style={{textAlign:"right"}}>
                      <div style={{fontFamily:"DM Sans,sans-serif",fontSize:13,fontWeight:700,color:layer.color}}>{p.confidence}%</div>
                      <div style={{fontFamily:"DM Sans,sans-serif",fontSize:9,color:"rgba(var(--ink-rgb),0.3)"}}>confidence</div>
                    </div>
                  </div>
                ))}
              </>}
              {layer.devices&&<>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                  {layer.devices.map(d=>(
                    <button key={d} style={{padding:"5px 10px",borderRadius:8,background:"rgba(var(--surface-rgb),0.05)",border:"1px solid rgba(var(--surface-rgb),0.12)",color:"rgba(var(--ink-rgb),0.6)",fontFamily:"DM Sans,sans-serif",fontSize:11,cursor:"pointer"}}>{d}</button>
                  ))}
                </div>
              ...
              <div style={{marginTop:12,padding:"8px 10px",borderRadius:8,background:layer.status==="active"?`${C.mint}12`:layer.status==="ready"?`${C.gold}12`:`rgba(var(--surface-rgb),0.04)`,border:`1px solid ${layer.status==="active"?C.mint:layer.status==="ready"?C.gold:"rgba(var(--surface-rgb),0.08)"}`}}>
                <div style={{fontFamily:"DM Sans,sans-serif",fontSize:11,color:layer.status==="active"?C.mint:layer.status==="ready"?C.gold:"rgba(var(--ink-rgb),0.4)"}}>
        ...
        <Card>
          ...
          {[
            ...
            <div key={i} style={{display:"flex",gap:12,alignItems:"flex-start",marginBottom:i<3?12:0,paddingBottom:i<3?12:0,borderBottom:i<3?"1px solid rgba(var(--surface-rgb),0.05)":"none"}}>
              <div style={{display:"flex",flexDirection:"column",alignItems:"center"}}>
                <div style={{width:28,height:28,borderRadius:"50%",background:step.done?`${step.color}30`:"rgba(var(--surface-rgb),0.05)",border:`2px solid ${step.done?step.color:"rgba(var(--surface-rgb),0.15)"}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  <span style={{fontSize:11,fontWeight:700,color:step.done?step.color:"rgba(var(--ink-rgb),0.3)",fontFamily:"DM Sans,sans-serif"}}>{step.done?"✓":i+1}</span>
                </div>
                {i<3&&<div style={{width:2,height:20,background:"rgba(var(--surface-rgb),0.06)",margin:"3px 0"}}/>}
              </div>
              <div style={{flex:1,paddingTop:4}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                  <span style={{fontFamily:"DM Sans,sans-serif",fontSize:12,fontWeight:600,color:step.done?C.cream:"rgba(var(--ink-rgb),0.5)"}}>{step.milestone}</span>
                ...
                <div style={{fontFamily:"DM Sans,sans-serif",fontSize:10,color:"rgba(var(--ink-rgb),0.4)"}}>{step.layers}</div>
              </div>
            </div>
          ))}
        </Card>
