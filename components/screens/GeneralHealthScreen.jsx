      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
        {metrics.map(m=>(
          <Card key={m.label} style={{padding:10,textAlign:"center"}}>
            ...
            <div style={{fontFamily:"DM Sans,sans-serif",fontSize:9,color:"rgba(var(--ink-rgb),0.4)",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:2}}>{m.label}</div>
        ...
        {metrics.map((m,i)=>(
          <div key={m.label} style={{display:"flex",gap:10,alignItems:"flex-start",marginBottom:i<metrics.length-1?12:0,paddingBottom:i<metrics.length-1?12:0,borderBottom:i<metrics.length-1?"1px solid rgba(var(--surface-rgb),0.05)":"none"}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontFamily:"DM Sans,sans-serif",fontSize:11,color:"rgba(var(--ink-rgb),0.6)",lineHeight:1.6}}>{m.tip}</div>
            </div>
          </div>
        ))}
      </Card>
      <Card style={{borderColor:`${C.gold}40`,background:`rgba(255,215,0,0.06)`}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          {["Apple Health","Oura Ring","Garmin"].map(w=>(
            <button key={w} style={{padding:"6px 12px",borderRadius:8,background:"rgba(var(--surface-rgb),0.06)",border:"1px solid rgba(var(--surface-rgb),0.12)",color:"rgba(var(--ink-rgb),0.7)",fontFamily:"DM Sans,sans-serif",fontSize:11,cursor:"pointer"}}>{w}</button>
          ))}
        </div>
      </Card>
    </div>
