  ...
  return (
    <div style={{height:"100%",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div style={{padding:"16px 16px 0"}}>
        ...
        <div style={{display:"flex",gap:6,marginBottom:14}}>
          {["tracker","calendar","insights"].map(tab=>(
            <button key={tab} onClick={()=>setActiveTab(tab)} style={{padding:"6px 14px",borderRadius:20,border:`1px solid ${activeTab===tab?phaseColor(today.phase):"rgba(var(--surface-rgb),0.1)"}`,background:activeTab===tab?`${phaseColor(today.phase)}20`:"transparent",color:activeTab===tab?phaseColor(today.phase):"rgba(var(--ink-rgb),0.5)",fontFamily:"DM Sans,sans-serif",fontSize:11,fontWeight:600,cursor:"pointer",textTransform:"capitalize"}}>
              {tab}
            </button>
          ))}
        </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"0 16px 28px",display:"flex",flexDirection:"column",gap:12}}>
        ...
        {activeTab==="tracker"&&<>
          <Card>
            <div style={{fontFamily:"DM Sans,sans-serif",fontSize:10,color:hasCycleData?C.mint:"rgba(var(--ink-rgb),0.4)",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:10}}>{hasCycleData?"Cycle Day":"Cycle Day · sample data"}</div>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
              <button onClick={()=>setCurrentDay(Math.max(1,currentDay-1))} style={{width:32,height:32,borderRadius:"50%",background:"rgba(var(--surface-rgb),0.08)",border:"none",color:C.pearl,fontSize:16,cursor:"pointer"}}>‹</button>
              ...
              <button onClick={()=>setCurrentDay(Math.min(35,currentDay+1))} style={{width:32,height:32,borderRadius:"50%",background:"rgba(var(--surface-rgb),0.08)",border:"none",color:C.pearl,fontSize:16,cursor:"pointer"}}>›</button>
            </div>
            <div style={{height:6,borderRadius:3,background:"rgba(var(--surface-rgb),0.08)",overflow:"hidden",marginBottom:8}}>
                ...
                <div key={item.label} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 10px",borderRadius:10,background:`${item.color}12`,border:`1px solid ${item.color}25`}}>
                  <div style={{display:"flex",gap:8,alignItems:"center"}}>
                    ...
                    <div>
                      ...
                      <div style={{fontFamily:"DM Sans,sans-serif",fontSize:9,color:"rgba(var(--ink-rgb),0.4)"}}>{[item.days, item.conf!=null?`AI: ${item.conf}%`:null].filter(Boolean).join(" · ")||"—"}</div>
                    </div>
                  </div>
          ...
          <Card style={{borderColor:`${C.vaginal}30`,background:`rgba(233,30,140,0.05)`}}>
            ...
            <div style={{fontFamily:"DM Sans,sans-serif",fontSize:12,color:"rgba(var(--ink-rgb),0.75)",lineHeight:1.7}}>
              ...
              <div key={g.category} style={{marginBottom:12}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                  {g.items.map(s=>{
                    ...
                    return (
                      <button key={s.id} onClick={()=>toggleSymptom(s.id)} style={{padding:"5px 10px",borderRadius:16,border:`1px solid ${on?g.color:"rgba(var(--surface-rgb),0.12)"}`,background:on?`${g.color}22`:"transparent",color:on?g.color:"rgba(var(--ink-rgb),0.55)",fontFamily:"DM Sans,sans-serif",fontSize:10,cursor:"pointer",display:"flex",gap:4,alignItems:"center"}}>
            ...
            {[
              ...
              <div key={h.label} style={{marginBottom:10}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                  <span style={{fontFamily:"DM Sans,sans-serif",fontSize:11,color:"rgba(var(--ink-rgb),0.7)"}}>{h.label}</span>
                ...
                <div style={{height:5,borderRadius:3,background:"rgba(var(--surface-rgb),0.06)"}}>
                  <div style={{width:`${h.value}%`,height:"100%",borderRadius:3,background:h.color,filter:`drop-shadow(0 0 3px ${h.color})`}}/>
                </div>
              </div>
            ))}
            <div style={{fontFamily:"DM Sans,sans-serif",fontSize:10,color:"rgba(var(--ink-rgb),0.35)",marginTop:4}}>Estimates from Layers 1 & 2 + biome inference. Add BBT for higher precision →</div>
          </Card>
          )}
        </>}
          ...
          <Card>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:4}}>
              {cal.days.map((c,i)=>{
                ...
                return (
                  <div key={day} style={{aspectRatio:"1",borderRadius:8,background:isToday?C.coral:isPeriod?`${C.rose}40`:isOv?`${C.gold}40`:isFertile?`${C.mint}25`:"rgba(var(--surface-rgb),0.04)",display:"flex",alignItems:"center",justifyContent:"center",border:isToday?`2px solid ${C.coral}`:isPeriod?`1px solid ${C.rose}50`:"none"}}>
                    <span style={{fontFamily:"DM Sans,sans-serif",fontSize:10,fontWeight:isToday?700:400,color:isToday?"white":isPeriod?C.rose:isOv?C.gold:"rgba(var(--ink-rgb),0.7)"}}>{day}</span>
                  </div>
                );
              })}
            </div>
            <div style={{display:"flex",gap:12,marginTop:12,flexWrap:"wrap"}}>
              {[{c:C.rose,l:"Period"},{c:C.gold,l:"Ovulation"},{c:C.mint,l:"Fertile"},{c:C.coral,l:"Today"}].map(x=>(
                <div key={x.l} style={{display:"flex",alignItems:"center",gap:5}}><div style={{width:8,height:8,borderRadius:2,background:x.c}}/><span style={{fontFamily:"DM Sans,sans-serif",fontSize:9,color:"rgba(var(--ink-rgb),0.5)"}}>{x.l}</span></div>
              ))}
            </div>
            {!hasCycleData&&<div style={{marginTop:10,fontFamily:"DM Sans,sans-serif",fontSize:10,color:"rgba(var(--ink-rgb),0.45)"}}>Log a period in the Tracker tab to see your predicted period, ovulation and fertile window here.</div>}
          </Card>
        </>}
            ...
            {insights.map((item,i)=>(
              <div key={item.label} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:i<4?"1px solid rgba(var(--surface-rgb),0.05)":"none"}}>
                <div>
                  ...
                  <div style={{fontFamily:"DM Sans,sans-serif",fontSize:10,color:"rgba(var(--ink-rgb),0.45)"}}>{item.detail}</div>
                </div>
